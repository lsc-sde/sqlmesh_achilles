...
MODEL (
  name @temp_schema.achilles__tempResults_1816,
  kind FULL,
  cron '@daily'
);
...
from
  (
    ...
    from `@temp_schema`.`achilles__statsView_1816` as s
    inner join
      `@temp_schema`.`achilles__statsView_1816` as p
      on
...
inner join
  `@temp_schema`.`achilles__overallStats_1816` as o
